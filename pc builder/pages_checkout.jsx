...
const STEPS_CK = ['Cart Review', 'Shipping', 'Payment', 'Confirmation'];

const CheckoutPage = () => {
  const { cart, setPage, clearCart } = React.useContext(window.AppContext);
  ...
  const validate = () => {
    ...
    if (step === 2) {
      if (payment.method === 'card') {
        if (!payment.cardNumber || payment.cardNumber.replace(/\s/g,'').length < 16) e.cardNumber = 'Enter 16-digit card number';
  ...
  const handleNext = async () => {
    ...
    if (step === 2) {
      ...
      try {
        ...
        await window.PcApi.cartSync(cart.map(i => ({ product_id: i.id, qty: i.qty })));
  ...
  return (
    <div style={ckStyles.section}>
      ...
      <div style={ckStyles.formGrid}>
        <Field label="First Name" value={data.firstName} onChange={v=>set('firstName',v)} error={errors.firstName} />
        <Field label="Last Name" value={data.lastName} onChange={v=>set('lastName',v)} error={errors.lastName} />
        <Field label="Email" type="email" value={data.email} onChange={v=>set('email',v)} error={errors.email} full />
        <Field label="Phone" type="tel" value={data.phone} onChange={v=>set('phone',v)} />
        <Field label="Street Address" value={data.address} onChange={v=>set('address',v)} error={errors.address} full />
        <Field label="City" value={data.city} onChange={v=>set('city',v)} error={errors.city} />
        <Field label="ZIP / Postal Code" value={data.zip} onChange={v=>set('zip',v)} error={errors.zip} />
        <div>
      ...
      <div style={ckStyles.shippingMethods}>
        ...
        {[
          { key:'standard', label:'Standard Shipping', desc:'5-7 business days', price:'Free over $500' },
          { key:'express', label:'Express Shipping', desc:'2-3 business days', price:'$29.99' },
          { key:'overnight', label:'Overnight', desc:'Next business day', price:'$59.99' },
        ].map(m => (
  ...
  return (
    <div style={ckStyles.section}>
      ...
      <div style={ckStyles.payMethods}>
        {[['card','Credit / Debit Card'],['paypal','PayPal'],['crypto','Crypto']].map(([k,l])=>(
      ...
      {data.method === 'card' && (
        <div style={ckStyles.formGrid}>
          ...
          <Field label="Cardholder Name" value={data.cardName} onChange={v=>set('cardName',v)} error={errors.cardName} full />
          <div>
...
const OrderConfirmation = ({ orderNum, cart, total, shipping, setPage }) => (
  <div style={ckStyles.confirmPage}>
    ...
    <h1 style={ckStyles.confirmTitle}>Order Confirmed!</h1>
    ...
    <div style={{ display:'flex', gap:12 }}>
      ...
      <button style={ckStyles.btnSecondary} onClick={() => setPage('home')}>Continue Shopping</button>
    </div>
  </div>
);
