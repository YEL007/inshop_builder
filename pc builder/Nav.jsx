...
const Nav = ({ page, setPage, cartCount, searchQuery, setSearchQuery }) => {
  ...
  const { favorites } = React.useContext(window.AppContext);

  const navLinks = [
    { id: 'catalog', label: 'Components', sub: ['CPU', 'GPU', 'Motherboard', 'RAM', 'Storage', 'PSU', 'Cooling', 'Case'] },
    { id: 'prebuilt', label: 'Pre-Built PCs' },
    { id: 'peripherals', label: 'Peripherals' },
    { id: 'builder', label: 'PC Builder', highlight: true },
  ];
    ...
    <nav style={navStyles.nav}>
      ...
      <div style={navStyles.links}>
        {navLinks.map(link => (
          ...
          >
            {link.label}
            {link.highlight && <span style={navStyles.newBadge}>NEW</span>}
          </button>
        ))}
      </div>
