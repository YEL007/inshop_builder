...
const CATEGORY_LABELS = {
  cpu:'Processors', gpu:'Graphics Cards', motherboard:'Motherboards',
  ram:'Memory', storage:'Storage', psu:'Power Supplies',
  cooling:'Cooling', case:'Cases',
};
const ALL_CATEGORIES = ['all','cpu','gpu','motherboard','ram','storage','psu','cooling','case'];

const CatalogPage = ({ initialCategory }) => {
  const { setPage, addToCart, toggleFav, favorites, compareList, toggleCompare, dataLoaded } = React.useContext(window.AppContext);
      ...
      <div style={catStyles.pageHeader}>
        <div style={catStyles.pageHeaderInner}>
          <div style={catStyles.breadcrumb}>
            <span style={{ color:'#9f9f9f', cursor:'pointer' }} onClick={() => setPage('home')}>Home</span>
            <span style={{ color:'#3c3c3c' }}> / </span>
            <span style={{ color:'#ffffff' }}>{category === 'all' ? 'All Components' : CATEGORY_LABELS[category]}</span>
          </div>
          <h1 style={catStyles.pageTitle}>{category === 'all' ? 'All Components' : CATEGORY_LABELS[category]}</h1>
        </div>
      </div>
      ...
      <div style={catStyles.layout}>
        ...
        <aside style={catStyles.sidebar}>
          ...
          <div style={catStyles.filterGroup}>
            ...
            {ALL_CATEGORIES.map(c => (
              <button key={c} style={{ ...catStyles.filterBtn, ...(category===c ? catStyles.filterBtnActive : {}) }}
                ...
                {c === 'all' ? 'All Components' : CATEGORY_LABELS[c]}
              </button>
            ))}
          </div>
          ...
          <div style={catStyles.filterGroup}>
            ...
            <div style={{ display:'flex', gap:8, marginBottom:10 }}>
              <div style={catStyles.priceInput}>${priceRange[0]}</div>
              <span style={{ color:'#9f9f9f', alignSelf:'center' }}>—</span>
              <div style={catStyles.priceInput}>${priceRange[1]}</div>
            </div>
          ...
          {compareList.length > 0 && (
            <div style={catStyles.comparePanel}>
              <div style={catStyles.filterLabel}>COMPARE ({compareList.length}/3)</div>
              {compareList.map(p => (
              ...
              {compareList.length >= 2 && (
                <button style={catStyles.compareBtn} onClick={() => setPage('compare')}>Compare Now</button>
              )}
            </div>
          )}
        </aside>
        ...
        <main style={catStyles.main}>
          ...
          <div style={catStyles.toolbar}>
            <input placeholder="Search in results..." value={search} onChange={e=>setSearch(e.target.value)}
              style={catStyles.searchInput}/>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginLeft:'auto' }}>
              <span style={{ color:'#9f9f9f', fontSize:13 }}>{allProducts.length} products</span>
          ...
          {allProducts.length === 0 ? (
            <div style={catStyles.empty}>
              ...
              <div style={{ color:'#666666' }}>No products found</div>
            </div>
          ) : (
...
const ProductDetailPage = ({ product }) => {
  const { setPage, addToCart, toggleFav, favorites, addToBuild, currentUser } = React.useContext(window.AppContext);
    ...
    <div style={pdStyles.page}>
      ...
      <div style={pdStyles.layout}>
        ...
        <div style={{ display:'flex', flexDirection:'row', gap:16, height: 500 }}>
          ...
          <div style={{ ...pdStyles.imgArea, background:`linear-gradient(${catGrad[product.category]||'135deg,#242424,#2a2a2a'})`, overflow:'hidden', padding:0, flex: 1, minHeight: 0, height: '100%' }}>
            {product.images && product.images.length > 0 ? (
              ...
              <window.ProductVisual category={product.category} />
            )}
            {product.tags?.map(t=>(
        ...
        <div style={pdStyles.info}>
          ...
          <div style={pdStyles.price}>${product.price.toLocaleString()}</div>
          ...
          <div style={pdStyles.stockRow}>
            ...
            <span style={{ color: product.stock==='in_stock'?'#909090':'#909090', fontSize:14 }}>
              {product.stock==='in_stock'?'In Stock':'Low Stock — Order soon'}
            </span>
          </div>
      ...
      <div style={{ borderTop:'1px solid #3c3c3c', paddingTop:48, marginTop:8 }}>
        ...
        {currentUser ? (
          <div style={{ background:'#242424', border:'1px solid #3c3c3c', borderRadius:12, padding:24, marginBottom:32 }}>
            ...
            <textarea value={myComment} onChange={e=>setMyComment(e.target.value)} placeholder="Partagez votre expérience avec ce produit..."
...
// Compare Page
const ComparePage = () => {
  const { compareList, toggleCompare, setPage, addToCart } = React.useContext(window.AppContext);
          ...
          <div key={p.id} style={cmpStyles.productHeader}>
            ...
            <div style={{ height:80, display:'flex', alignItems:'center', justifyContent:'center' }}>
              {p.image
                ...
                : <window.ProductVisual category={p.category} />
              }
            </div>
...
Object.assign(window, { CatalogPage, ProductDetailPage, ComparePage, CATEGORY_LABELS });
