...
const CATEGORY_CARDS = [
  { id:'cpu', label:'Processors', icon:'🔲', desc:'CPU', color:'#e8793a' },
  { id:'gpu', label:'Graphics Cards', icon:'🎮', desc:'GPU', color:'#6c5ce7' },
  { id:'motherboard', label:'Motherboards', icon:'📟', desc:'Motherboard', color:'#00b894' },
  { id:'ram', label:'Memory', icon:'💾', desc:'RAM', color:'#fdcb6e' },
  { id:'storage', label:'Storage', icon:'⚡', desc:'SSD / HDD', color:'#74b9ff' },
  { id:'psu', label:'Power Supplies', icon:'🔌', desc:'PSU', color:'#fd79a8' },
  { id:'cooling', label:'Cooling', icon:'❄️', desc:'Cooling', color:'#55efc4' },
  { id:'case', label:'Cases', icon:'🖥️', desc:'Case', color:'#b2bec3' },
];
...
const HomePage = () => {
  const { setPage, cart, addToCart, favorites, toggleFav } = React.useContext(window.AppContext);
  const featured = [
    ...
    <div style={homeStyles.page}>
      ...
      <section style={homeStyles.hero}>
        ...
        <div style={homeStyles.heroContent}>
          <div style={homeStyles.heroEyebrow}>⚡ NEXT-GEN PERFORMANCE</div>
          <h1 style={homeStyles.heroTitle}>
            Build Your<br/>
            <span style={{ color:'#e8001d' }}>Dream PC</span>
          </h1>
          <p style={homeStyles.heroDesc}>
            Top-tier components, expert compatibility checking,<br/>
            and pre-built systems — all in one place.
          </p>
          ...
          <div style={homeStyles.heroStats}>
            {[['500+','Products'],['99.9%','Uptime'],['48h','Delivery'],['5★','Rating']].map(([n,l])=>(
      ...
      <section style={homeStyles.section}>
        <div style={homeStyles.sectionHeader}>
          <div>
            <div style={homeStyles.sectionEye}>SHOP BY CATEGORY</div>
            <h2 style={homeStyles.sectionTitle}>Components</h2>
          </div>
        </div>
        <div style={homeStyles.categoriesGrid}>
          {CATEGORY_CARDS.map(cat => (
            <button key={cat.id} style={homeStyles.catCard}
              ...
              <div style={homeStyles.catLabel}>{cat.label}</div>
              <div style={homeStyles.catDesc}>
                {(window.CATALOG[cat.id]||[]).length > 0
                  ? `${(window.CATALOG[cat.id]||[]).length} product${(window.CATALOG[cat.id]||[]).length > 1 ? 's' : ''}`
                  : cat.desc}
              </div>
            </button>
          ))}
      ...
      <section style={homeStyles.section}>
        <div style={homeStyles.sectionHeader}>
          <div>
            <div style={homeStyles.sectionEye}>HANDPICKED FOR YOU</div>
            <h2 style={homeStyles.sectionTitle}>Featured Products</h2>
          </div>
      ...
      <section style={homeStyles.ctaBanner}>
        ...
        <div style={{ position:'relative', zIndex:1 }}>
          <div style={homeStyles.ctaEye}>CUSTOM BUILDS</div>
          <h2 style={homeStyles.ctaTitle}>Not sure where to start?</h2>
          <p style={homeStyles.ctaDesc}>Our PC Builder guides you through every component with real-time compatibility checking and performance scoring.</p>
