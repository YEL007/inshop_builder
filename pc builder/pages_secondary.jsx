...
// ─── PRE-BUILT PCs ───────────────────────────────────────────────────────────
const PrebuiltPage = () => {
  const { setPage, addToCart } = React.useContext(window.AppContext);
            ...
            <div key={pc.id} style={{ ...secStyles.pbCard, cursor:'pointer' }}
              ...
              <div style={secStyles.pbBody}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                  ...
                  <div style={secStyles.pbPrice}>${pc.price.toLocaleString()}</div>
                </div>
    ...
    <div style={{ paddingTop: 64, padding: '64px 80px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:48, marginBottom:64 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column' }}>
          ...
          <div style={{ fontFamily:"'Space Grotesk',sans-serif", fontWeight:700, fontSize:40, color:'#ffffff', marginBottom:16 }}>${pc.price.toLocaleString()}</div>
          ...
          <button style={{ width:'100%', padding:'14px', background:'#e8001d', color:'#ffffff', border:'none', borderRadius:8, fontFamily:"'Space Grotesk',sans-serif", fontWeight:700, fontSize:15, cursor:'pointer', transition:'background 0.2s', marginBottom:12 }}
            ...
            Add to Cart — ${(pc.price * qty).toLocaleString()}
          </button>
        </div>
      </div>
...
const PERI_GROUPS = [
  {
    ...
    desc: 'Envoient des données vers l\'ordinateur',
  ...
  },
  {
    ...
    desc: 'Reçoivent des données de l\'ordinateur',
    ...
    tabs: [
      ...
      { key: 'speaker',  label: 'Haut-parleurs' },
      { key: 'headset',  label: 'Casque audio' },
    ],
  },
  {
    id: 'mixed',
    label: 'Périphériques E/S',
    ...
    tabs: [
      ...
      { key: 'external_hdd', label: 'Disque externe' },
      { key: 'network',      label: 'Carte réseau' },
    ],
  },
];
...
const PeripheralsPage = () => {
  const { setPage, addToCart, toggleFav, favorites } = React.useContext(window.AppContext);
...
const PeriProductCard = ({ product, visual, onAdd, onFav, onView, isFav }) => {
  ...
  const badge = product.tags?.includes('bestseller') ? 'BESTSELLER'
    : product.stock === 'low_stock' ? 'LOW STOCK'
    : product.stock === 'out_of_stock' ? 'OUT OF STOCK' : null;
  ...
  return (
    <div style={{ background:'#242424', border:`1px solid ${hov ? color : '#3c3c3c'}`, borderRadius:16, overflow:'hidden', display:'flex', flexDirection:'column', transition:'all 0.25s', position:'relative' }}
      ...
      <div style={{ height:190, background:`linear-gradient(135deg, ${color}14, #212121)`, overflow:'hidden', cursor:'pointer' }} onClick={onView}>
        <window.ImageCarousel images={product.images} category={product.category} />
      </div>
      ...
      <div style={{ padding:'18px', flex:1, display:'flex', flexDirection:'column' }} onClick={onView}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14 }}>
          ...
          <div style={{ fontFamily:"'Space Grotesk',sans-serif", fontWeight:700, fontSize:19, color:'#e8001d', flexShrink:0 }}>${product.price.toLocaleString()}</div>
        </div>
...
// ─── CART ─────────────────────────────────────────────────────────────────────
const CartPage = () => {
  const { cart, removeFromCart, updateQty, setPage } = React.useContext(window.AppContext);
  ...
  if (cart.length === 0) return (
    <div style={{ paddingTop: 64, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '80vh', gap: 16 }}>
      ...
      <div style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 600, fontSize: 20, color: '#9f9f9f' }}>Your cart is empty</div>
      <button style={{ ...window.homeStyles.heroBtnPrimary, marginTop: 8 }} onClick={() => setPage('catalog')}>Browse Components</button>
    </div>
  );

  return (
    <div style={{ paddingTop: 64, padding: '64px 80px' }}>
      <h1 style={secStyles.pageTitle2}>Your Cart <span style={{ color: '#9f9f9f', fontWeight: 400, fontSize: 20 }}>({cart.length} item{cart.length !== 1 ? 's' : ''})</span></h1>
      <div style={secStyles.cartLayout}>
          ...
          {cart.map(item => (
            <div key={item.cartId} style={secStyles.cartItem}>
              ...
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                <div style={{ color: '#ffffff', fontWeight: 700, fontSize: 20, fontFamily: "'Space Grotesk',sans-serif" }}>${(item.price * item.qty).toLocaleString()}</div>
                {item.qty > 1 && <div style={{ color: '#9f9f9f', fontSize: 12 }}>${item.price} each</div>}
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={secStyles.cartSummary}>
          <div style={secStyles.summaryTitle}>Order Summary</div>
          <div style={secStyles.summaryRow}>
            <span style={{ color: '#666666' }}>Subtotal</span>
            <span style={{ color: '#ffffff' }}>${subtotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
          </div>
          {couponApplied && (
            <div style={secStyles.summaryRow}>
              <span style={{ color: '#909090' }}>Coupon (INSHOP10)</span>
              <span style={{ color: '#909090' }}>−${discount.toLocaleString()}</span>
            </div>
          )}
          <div style={secStyles.summaryRow}>
            <span style={{ color: '#666666' }}>Shipping</span>
            <span style={{ color: shipping === 0 ? '#909090' : '#ffffff' }}>{shipping === 0 ? 'Free' : '$' + shipping.toFixed(2)}</span>
          </div>
          {subtotal < 500 && <div style={{ color: '#9f9f9f', fontSize: 11, marginBottom: 12 }}>Free shipping on orders over $500</div>}
          ...
          <div style={{ ...secStyles.summaryRow, marginBottom: 20 }}>
            ...
            <span style={{ color: '#e8001d', fontWeight: 700, fontSize: 24, fontFamily: "'Space Grotesk',sans-serif" }}>${total.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
          </div>
          {/* Coupon */}
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            ...
            <button style={secStyles.couponBtn}
              onClick={() => { if (coupon.toUpperCase() === 'INSHOP10') setCouponApplied(true); }}>Apply</button>
          </div>
          ...
          <button style={secStyles.continueBtn} onClick={() => setPage('catalog')}>Continue Shopping</button>
        </div>
      </div>
    </div>
  );
...
const AuthForm = ({ onSuccess }) => {
  ...
  const handle = async () => {
    ...
    try {
      ...
      setError(e.message || (mode === 'login' ? 'Invalid email or password' : 'Registration failed'));
    } finally { setLoading(false); }
  };

  const inp = { ...secStyles.fieldInput, marginBottom:0 };
  return (
    <div style={{ maxWidth:400, margin:'60px auto', background:'#242424', border:'1px solid #3c3c3c', borderRadius:16, padding:'36px 32px' }}>
      <div style={{ fontFamily:"'Space Grotesk',sans-serif", fontWeight:700, fontSize:22, color:'#ffffff', marginBottom:6 }}>
        {mode === 'login' ? 'Sign In' : 'Create Account'}
      </div>
      <div style={{ color:'#666666', fontSize:13, marginBottom:28 }}>
        {mode === 'login' ? 'Access your orders, favorites and profile.' : 'Join InShop Builder.'}
      </div>
      {mode === 'register' && (
        <div style={secStyles.profileField}>
          <label style={secStyles.fieldLabel}>Full Name</label>
          <input value={form.name} onChange={e=>set('name',e.target.value)} placeholder="John Doe" style={inp} />
        </div>
      )}
      <div style={secStyles.profileField}>
        <label style={secStyles.fieldLabel}>Email</label>
        <input type="email" value={form.email} onChange={e=>set('email',e.target.value)} placeholder="you@example.com" style={inp} />
      </div>
      <div style={secStyles.profileField}>
        <label style={secStyles.fieldLabel}>Password</label>
        <input type="password" value={form.password} onChange={e=>set('password',e.target.value)} placeholder="••••••••" style={inp}
          onKeyDown={e=>e.key==='Enter'&&handle()} />
      </div>
      {error && <div style={{ color:'#cc4444', fontSize:12, marginBottom:12, padding:'8px 12px', background:'#cc444415', borderRadius:6 }}>{error}</div>}
      <button style={{ ...secStyles.saveBtn, width:'100%', padding:'13px', fontSize:15, opacity:loading?0.7:1 }}
        onClick={handle} disabled={loading}>
        {loading ? 'Please wait…' : mode === 'login' ? 'Sign In' : 'Create Account'}
      </button>
      <div style={{ textAlign:'center', marginTop:20, color:'#666666', fontSize:13 }}>
...
const ProfileTab = ({ currentUser, setCurrentUser }) => {
  ...
  return (
    <div style={{ maxWidth: 540 }}>
      <h2 style={secStyles.tabTitle}>Profile Information</h2>
      
      <div style={secStyles.profileField}>
        <label style={secStyles.fieldLabel}>Full Name</label>
        <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} 
          placeholder="Enter your name" style={secStyles.fieldInput} />
      </div>

      <div style={secStyles.profileField}>
        <label style={secStyles.fieldLabel}>Email Address</label>
        <input value={currentUser.email} disabled style={{ ...secStyles.fieldInput, opacity: 0.6, cursor: 'not-allowed' }} />
        <div style={{ fontSize: 11, color: '#666', marginTop: 4 }}>Email cannot be changed.</div>
      </div>

      <div style={secStyles.profileField}>
        <label style={secStyles.fieldLabel}>Phone Number</label>
        <input value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} 
          placeholder="Enter phone number" style={secStyles.fieldInput} />
      </div>

      <div style={secStyles.profileField}>
        <label style={secStyles.fieldLabel}>Shipping Address</label>
        <textarea value={form.street} onChange={e => setForm({ ...form, street: e.target.value })} 
          placeholder="Enter your shipping address" style={{ ...secStyles.fieldInput, height: 80, resize: 'vertical' }} />
      </div>
...
const UserPage = ({ initialTab }) => {
  const { favorites, setPage, addToCart, currentUser, setCurrentUser } = React.useContext(window.AppContext);
  ...
  if (!currentUser) {
    return (
      <div style={secStyles.page}>
        <div style={secStyles.pageHeader}>
          <div style={secStyles.headerEye}>MY ACCOUNT</div>
          <h1 style={{ ...secStyles.pageTitle, margin:0 }}>Sign In</h1>
          <p style={secStyles.pageDesc}>Access your orders, favorites, and profile.</p>
        </div>
  ...
  return (
    <div style={secStyles.page}>
      ...
      <div style={{ display:'flex', borderBottom:'1px solid #3c3c3c', padding:'0 80px' }}>
        {tabs.map(t => (
          <button key={t} style={{ ...secStyles.userTab, ...(tab===t?secStyles.userTabActive:{}) }} onClick={() => setTab(t)}>
            {t.charAt(0).toUpperCase() + t.slice(1)}
            {t === 'favorites' && favProducts.length > 0 && <span style={secStyles.tabCount}>{favProducts.length}</span>}
            {t === 'orders' && orders.length > 0 && <span style={secStyles.tabCount}>{orders.length}</span>}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding:'40px 80px' }}>
        ...
        {tab === 'orders' && (
          <div>
            <h2 style={secStyles.tabTitle}>Order History</h2>
            {ordersLoading ? (
              <div style={{ color:'#666666', padding:'40px 0' }}>Loading orders…</div>
            ) : orders.length === 0 ? (
              <div style={{ textAlign:'center', padding:'60px 20px' }}>
                <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#3c3c3c" strokeWidth="1.5" style={{ marginBottom:16 }}>
                  <path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/>
                </svg>
                <div style={{ color:'#9f9f9f', fontSize:15, marginBottom:8 }}>No orders yet</div>
                <div style={{ color:'#666666', fontSize:13, marginBottom:20 }}>Your confirmed orders will appear here</div>
                <button style={secStyles.addBtn} onClick={() => setPage('catalog')}>Start Shopping →</button>
              </div>
            ) : orders.map(order => {
                  ...
                  <div style={secStyles.orderItems}>
                    {(order.items||[]).map((item, i) => (
                      <div key={i} style={secStyles.orderItemRow}>
                        ...
                        <span style={{ color:'#9f9f9f', fontSize:13 }}>×{item.qty} · ${Number(item.price||0).toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                  <div style={{ padding:'12px 20px', borderTop:'1px solid #2a2a2a', display:'flex', gap:10 }}>
                    <button style={secStyles.orderActionBtn}>Track Order</button>
                    <button style={secStyles.orderActionBtn}>View Invoice</button>
                  </div>
                </div>
              );
            })}
        ...
        {tab === 'favorites' && (
          <div>
            <h2 style={secStyles.tabTitle}>Saved Items <span style={{ color:'#9f9f9f', fontWeight:400 }}>({favProducts.length})</span></h2>
            {favProducts.length === 0 ? (
              <div style={{ textAlign:'center', padding:'60px 20px' }}>
                <div style={{ color:'#9f9f9f', fontSize:16, marginBottom:16 }}>No saved items yet</div>
                <button style={window.homeStyles.heroBtnSecondary} onClick={() => setPage('catalog')}>Browse Products</button>
              </div>
            ) : (
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16 }}>
                {favProducts.map(p => (
                  <window.ProductCard key={p.id} product={p}
                    onAdd={() => addToCart(p)}
                    onFav={() => {}}
                    onView={() => setPage('product', { product:p })}
                    isFav={true} />
        ...
        {tab === 'settings' && (
          <div style={{ maxWidth:540 }}>
            <h2 style={secStyles.tabTitle}>Account Settings</h2>
            {[['Email Notifications','Receive order updates and deals'],['Newsletter','Weekly product highlights'],['Two-Factor Auth','Enhanced account security']].map(([title, desc]) => (
            ...
            <div style={secStyles.dangerZone}>
              <div style={{ color:'#cc4444', fontWeight:600, fontSize:14, marginBottom:8 }}>Danger Zone</div>
              <button style={secStyles.deleteBtn} onClick={handleLogout}>Sign Out</button>
            </div>
          </div>
        )}
      </div>
