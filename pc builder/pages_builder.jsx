...
const WIZARD_STEPS = [
  { key:'cpu',         num:1, label:'Processeur',    labelEN:'Processor',    catalog:'cpu' },
  { key:'motherboard', num:2, label:'Carte Mère',     labelEN:'Motherboard',  catalog:'motherboard' },
  { key:'ram',         num:3, label:'Mémoire RAM',    labelEN:'Memory',       catalog:'ram' },
  { key:'gpu',         num:4, label:'Carte Graphique',labelEN:'Graphics Card',catalog:'gpu' },
  { key:'storage',     num:5, label:'Stockage',       labelEN:'Storage',      catalog:'storage' },
  { key:'psu',         num:6, label:'Alimentation',   labelEN:'Power Supply', catalog:'psu' },
  { key:'case',        num:7, label:'Boîtier',        labelEN:'Case',         catalog:'case' },
  { key:'cooling',     num:8, label:'Refroidissement', labelEN:'Cooling',     catalog:'cooling' },
];
...
const BuilderPage = () => {
  const { build, setBuild, addToCart, dataLoaded } = React.useContext(window.AppContext);
    ...
    <div style={wiz.page}>
      ...
      <div style={wiz.titleBar}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div>
            <h1 style={wiz.title}>PC Builder</h1>
            <p style={wiz.subtitle}>Select components step by step — compatibility is verified automatically.</p>
          </div>
        </div>
      </div>
      ...
      <div style={wiz.body}>
        ...
        <aside style={wiz.sidebar}>
            ...
            {WIZARD_STEPS.map((s, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span style={wiz.stepLabel}>{s.label}</span>
                  {isActive && (
                  ...
                  {isDone && !isActive && (
                    <span style={wiz.stepPrice}>${build[s.key].price.toLocaleString()}</span>
                  )}
                </button>
              );
            })}
          ...
          <div style={wiz.summaryBox}>
            ...
            <div style={wiz.summaryTotal}>${total.toLocaleString()}</div>
            <button
        ...
        <main style={wiz.main}>
          ...
          <div style={wiz.stepHeader}>
            <div>
              <h2 style={wiz.stepTitle}>{step.labelEN}</h2>
              <p style={wiz.stepHint}>
                {build[step.key]
                  ? <><span style={{ color:'#e8001d' }}>✓ Selected: </span>{build[step.key].name}</>
                  : `Choose your ${step.labelEN.toLowerCase()}`}
              </p>
            </div>
    ...
    >
      ...
      <div style={{ padding:'16px', flex:1, display:'flex', flexDirection:'column' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
          ...
          <div style={{ fontFamily:"'Space Grotesk',sans-serif", fontWeight:700, fontSize:17, color:'#e8001d', flexShrink:0 }}>${product.price.toLocaleString()}</div>
        </div>
