...
const App = () => {
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case 'catalog': return <window.CatalogPage initialCategory={pageParams.category || 'all'} />;
      case 'prebuilt': return <window.PrebuiltPage />;
      case 'peripherals': return <window.PeripheralsPage />;
      case 'builder': return <window.BuilderPage />;
      case 'cart': return <window.CartPage />;
      case 'checkout': return <window.CheckoutPage />;
      case 'user': return <window.UserPage initialTab={pageParams.tab} />;
      case 'product': return pageParams.product ? <window.ProductDetailPage product={pageParams.product} /> : <window.HomePage />;
      case 'prebuilt-detail': return pageParams.product ? <window.PrebuiltDetailPage product={pageParams.product} /> : <window.PrebuiltPage />;
      case 'compare': return <window.ComparePage />;
  ...
  return (
    <AppContext.Provider value={ctxValue}>
      <div style={{ minHeight: '100vh', background: tweaks.bgBase || '#1a1a1a', fontFamily: tweaks.fontHeading }}>
        <window.Nav
        ...
        <footer style={appStyles.footer}>
          <div style={appStyles.footerTop}>
            ...
            <div style={appStyles.footerLinks}>
              {[['Components', ['CPU', 'GPU', 'Motherboards', 'Memory', 'Storage']],
              ['Products', ['Pre-Built PCs', 'Peripherals', 'PC Builder', 'Compare']],
              ['Account', ['My Orders', 'Favorites', 'Profile', 'Settings']]].map(([title, links]) => (
                <div key={title}>
          ...
          <div style={appStyles.footerBottom}>
            <span style={{ color: '#d0d0d0' }}>© 2026 InShop Builder. All rights reserved.</span>
            <span style={{ color: '#d0d0d0' }}>Made with precision.</span>
          </div>
        </footer>
